...
def load_bid_data():
    ...
    try:
        ...
        contractor_row = next(csv.reader([lines[contractor_row_index]], skipinitialspace=True))
        ...
        # Identify all unique section titles
        section_list = [str(s) for s in df['Section Title'].dropna().unique() if str(s).strip()]
        ...
        for idx, row in df.iterrows():
            ...
            for contractor, cols in contractor_map.items():
                ...
                if quantity_col is None or idx+1 >= len(df):
                    ...
                    if pd.notna(quantity_value) and str(quantity_value).strip() != '':
                        try:
                            ...
                            parse_errors.append(f"Row {idx+2}: Contractor '{contractor}' quantity value '{quantity_value}' is not numeric.")
                ...
                if unit_price_col is None or idx+1 >= len(df):
                    ...
                    if pd.notna(unit_price_value) and str(unit_price_value).strip() != '':
                        try:
                            float(str(unit_price_value).replace('$', '').replace(',', ''))
                        except Exception:
                            parse_errors.append(f"Row {idx+2}: Contractor '{contractor}' unit price value '{unit_price_value}' is not numeric.")
                ...
                if extension_col is None or idx+1 >= len(df):
                    print(f"[DEBUG] Skipping contractor '{contractor}' at row {idx+1}: extension_col index invalid ({extension_col}) or row out of bounds.")
                else:
                    extension_value = df.iloc[idx+1, extension_col]
                    if pd.notna(extension_value) and str(extension_value).strip() != '':
                        try:
                            float(str(extension_value).replace('$', '').replace(',', ''))
                        except Exception:
                            parse_errors.append(f"Row {idx+2}: Contractor '{contractor}' extension value '{extension_value}' is not numeric.")
...
def get_total_bids(df, contractor_map):
    base_bid_row = df[df["Section Title"].astype(str).str.contains("Base Bid Total:", na=False)]
