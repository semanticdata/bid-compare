    ...
    try:
        ...
        for idx, row in df.iterrows():
            if isinstance(row["Section Title"], str) and (
                "Mill and Overlay" in row["Section Title"]
                or "section - required" in row["Section Title"]
            ):
        ...
        df.attrs["extension_cols"] = [
            col for col in original_cols if "Extension" in col
        ]
        df.attrs["unit_price_cols"] = [
            col for col in original_cols if "Unit Price" in col
        ]
...
def main():
    ...
    try:
        ...
        st.header("Bid Analysis by Section")

        # Create DataFrame for plotting
        plot_data = []
        for section, totals in section_totals.items():
            for contractor, amount in totals.items():
                section_name = section.replace(" section - required", "").replace(
                    "S.3887 2025 ", ""
                )
                plot_data.append(
                    {
        ...
        # Get unique sections and contractors
        sections = plot_df["Section"].unique()
        contractors = plot_df["Contractor"].unique()

        # Set width of bars and positions
        bar_width = 0.25
        r = np.arange(len(contractors))

        # Plot bars for each section
        for idx, section in enumerate(sections):
            section_data = plot_df[plot_df["Section"] == section]
            section_amounts = [
                (
                    section_data[section_data["Contractor"] == c]["Amount"].iloc[0]
                    if not section_data[section_data["Contractor"] == c].empty
                    else 0
                )
            ...
            bars = plt.bar(
                position, section_amounts, bar_width, label=section, alpha=0.8
            )

            # Add value labels on top of bars
            for i, v in enumerate(section_amounts):
                plt.text(
                    position[i],
                    v,
                    f"${v:,.0f}",
        ...
        # Customize the plot
        plt.xlabel("Contractors")
        plt.ylabel("Bid Amount ($)")
        plt.title("Bid Amounts by Section and Contractor")
        plt.xticks(
            [r + bar_width for r in range(len(contractors))],
            contractors,
            rotation=45,
            ha="right",
        )
        ...
        for contractor in sorted(contractors):
            row = {
                "Contractor": contractor,
                "S.3887 2025 Mill and Overlay": comparison_data[contractor].get(
                    "S.3887 2025 Mill and Overlay", 0
                ),
                "Alternate 1": comparison_data[contractor].get(
                    "Alternate 1 section - required", 0
                ),
                "Alternate 2": comparison_data[contractor].get(
                    "Alternate 2 section - required", 0
                ),
            }
            # Calculate total
            row["Total"] = sum(
                value for key, value in row.items() if key != "Contractor"
            )
            rows.append(row)
